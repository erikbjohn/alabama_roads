...
bazel-bin/tensorflow/examples/image_retraining/retrain --image_dir /home/ebjohnson5/pkg.data/alabama_roads/raw/training_data
...
```{r Classification, eval=TRUE, echo=TRUE}
  ...
  for (pic in randomPics){
    ...
    cat('\n')
    if(file.exists(dt.classify.location)){
      dt.classify <- rbindlist(list(dt.classify, dt.classes.new), use.names = TRUE)
